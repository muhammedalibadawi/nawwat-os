...
export default function InventoryScreen() {
    ...
    const alertsView = useMemo(() => {
        return enrichedItems.filter((i) => i.status === 'out' || i.status === 'low');
    }, [enrichedItems]);
    ...
    return (
        <div className="p-6 md:p-8 h-full overflow-y-auto bg-[#F4F7FC] font-arabic text-[#071C3B]">
            ...
            {!tabLoading && !tabError && activeTab === 'alerts' && (
                <div className="bg-white rounded-2xl p-4 border border-[#071C3B]/10">
                    {alertsView.length === 0 ? <div className="p-8 text-center text-[#071C3B]/60 font-bold">لا توجد تنبيهات نشطة</div> : (
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead className="text-xs uppercase text-[#071C3B]/60"><tr><th className="py-3 text-start">المنتج</th><th className="py-3 text-start">الكمية</th><th className="py-3 text-start">الحد الأدنى</th><th className="py-3 text-start">التنبيه</th><th className="py-3 text-start">إجراء</th></tr></thead>
                                <tbody className="divide-y divide-gray-100">
                                    {alertsView.map((row) => (
                                        <tr key={row.id}>
                ...
                <div className="fixed inset-0 bg-[#071C3B]/35 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <form onSubmit={saveProduct} className="w-full max-w-2xl bg-white rounded-2xl p-6 border border-[#071C3B]/10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <input className="px-3 py-2 rounded-lg border border-gray-200" placeholder="Barcode" value={productForm.barcode} onChange={(e) => setProductForm({ ...productForm, barcode: e.target.value })} />
                            <input className="px-3 py-2 rounded-lg border border-gray-200" placeholder="Category ID" value={productForm.category_id} onChange={(e) => setProductForm({ ...productForm, category_id: e.target.value })} />
                ...
                <div className="fixed inset-0 bg-[#071C3B]/35 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <form onSubmit={saveMovement} className="w-full max-w-xl bg-white rounded-2xl p-6 border border-[#071C3B]/10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <input className="px-3 py-2 rounded-lg border border-gray-200" placeholder="Batch No (اختياري)" value={movementForm.batch_no} onChange={(e) => setMovementForm({ ...movementForm, batch_no: e.target.value })} />
                            <input type="date" className="px-3 py-2 rounded-lg border border-gray-200" placeholder="Expiry Date (اختياري)" value={movementForm.expiry_date} onChange={(e) => setMovementForm({ ...movementForm, expiry_date: e.target.value })} />
