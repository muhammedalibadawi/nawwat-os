-- Extend commercial_contract_version_create_and_set_current: optional term rows in the same transaction
-- as version creation + is_current switching + current_version_id update (promote path cannot attach terms here).
BEGIN;

DROP FUNCTION IF EXISTS public.commercial_contract_version_create_and_set_current(
    UUID,
    UUID,
    INT,
    TEXT,
