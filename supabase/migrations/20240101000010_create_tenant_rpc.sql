-- ============================================================
-- NawwatOS — Migration 20240101000010_create_tenant_rpc
...
BEGIN
    ...
    INSERT INTO public.tenants (name, slug, sector, country, is_active, plan, created_at, updated_at)
    VALUES (p_tenant_name, p_tenant_slug, p_sector, p_country, true, 'starter', now(), now())
    RETURNING id INTO v_tenant_id;

    -- 2. Create the default "Main Branch"
    INSERT INTO public.branches (tenant_id, name, country, is_active, is_default, created_at, updated_at)
    VALUES (v_tenant_id, 'Main Branch', p_country, true, true, now(), now())
    ...
    VALUES (p_auth_id, v_tenant_id, v_branch_id, p_full_name, p_email, true, now(), now())
    ...
    -- 4. Create public.profiles entry (To satisfy AuthContext.tsx stub safely)
    INSERT INTO public.profiles (user_id, full_name, created_at, updated_at)
    VALUES (v_user_id, p_full_name, now(), now())
