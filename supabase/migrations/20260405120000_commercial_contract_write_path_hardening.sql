-- Harden contract version write surface:
-- 1) Trigger: block direct authenticated updates to commercial_contracts.current_version_id (RPC sets session flag).
-- 2) RLS: remove INSERT/UPDATE on commercial_contract_versions + commercial_contract_version_terms for authenticated;
--    writes go through SECURITY DEFINER RPCs only (postgres owner bypasses RLS).
BEGIN;

CREATE OR REPLACE FUNCTION public.trg_guard_commercial_contracts_pointer_update()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
    IF OLD.current_version_id IS DISTINCT FROM NEW.current_version_id THEN
        IF current_setting('app.allow_cc_pointer_update', true) IS DISTINCT FROM '1' THEN
            RAISE EXCEPTION
                'commercial_contracts.current_version_id cannot be updated directly; use commercial_contract_version_create_and_set_current';
        END IF;
    END IF;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_guard_commercial_contracts_pointer_update ON public.commercial_contracts;
CREATE TRIGGER trg_guard_commercial_contracts_pointer_update
    BEFORE UPDATE OF current_version_id ON public.commercial_contracts
    FOR EACH ROW
    WHEN (OLD.current_version_id IS DISTINCT FROM NEW.current_version_id)
    EXECUTE FUNCTION public.trg_guard_commercial_contracts_pointer_update();
...
BEGIN
    ...
    PERFORM set_config('app.allow_cc_pointer_update', '1', true);

    IF p_terms IS NOT NULL AND jsonb_typeof(p_terms) <> 'array' THEN
        RAISE EXCEPTION 'p_terms must be a JSON array';
    END IF;

    IF p_promote_version_id IS NOT NULL AND jsonb_array_length(COALESCE(p_terms, '[]'::jsonb)) > 0 THEN
        RAISE EXCEPTION 'Terms cannot be attached when promoting an existing version';
    END IF;
...
DROP POLICY IF EXISTS "commercial_contract_versions: jwt_insert_admin" ON public.commercial_contract_versions;
DROP POLICY IF EXISTS "commercial_contract_versions: jwt_update_admin" ON public.commercial_contract_versions;

DROP POLICY IF EXISTS "commercial_contract_version_terms: jwt_insert_admin" ON public.commercial_contract_version_terms;
DROP POLICY IF EXISTS "commercial_contract_version_terms: jwt_update_admin" ON public.commercial_contract_version_terms;
