-- =============================================================================
-- NawwatOS — fix_tenant_v2: create_nawwat_tenant + fk_ur_user safety
--
-- Schema (من nawwat_schema_v5_1_FINAL.sql):
--   user_roles.fk_ur_user: FOREIGN KEY (tenant_id, user_id)
--     REFERENCES public.users (tenant_id, id) ON DELETE CASCADE
--   user_roles.fk_ur_role: FOREIGN KEY (tenant_id, role_id)
--     REFERENCES public.roles (tenant_id, id) ON DELETE CASCADE
--   UNIQUE (tenant_id, user_id, role_id, branch_id)
--
-- المشكلة: UPSERT على users يغيّر tenant_id بينما user_roles ما زال يشير إلى
--   (tenant_id_قديم, user_id) → يخالف المفتاح المركب أو يمنع التحديث.
--
-- الحل:
--   1) قبل تحديث صف users: حذف كل user_roles لهذا user_id (تفريغ يدوي قبل
--      تغيير tenant — ليس DELETE قبل INSERT لنفس صف المالك فقط).
--   2) users: INSERT ... ON CONFLICT (auth_id) DO UPDATE — لا يُمس عمود id.
--   3) user_roles: INSERT ... ON CONFLICT (tenant_id, user_id, role_id, branch_id)
--      DO NOTHING — بدون DELETE مخصص قبل إدراج المالك.
-- =============================================================================
...
BEGIN
  ...
  -- مستخدم قائم: إزالة كل user_roles أولاً حتى لا يمنع تغيير tenant_id في users
  ...
  -- users: لا يُحدَّث id — فقط tenant_id, default_branch, أسماء، بريد
